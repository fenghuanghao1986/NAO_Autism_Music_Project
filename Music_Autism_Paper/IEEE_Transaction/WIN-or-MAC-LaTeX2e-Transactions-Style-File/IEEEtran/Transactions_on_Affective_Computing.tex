...
\title{NO Answer from Face?\\Continuous Emotion Detection In EDA Signals}
%
%
...
The EDA data recorded using the SC sensors are categorized as non-stationery signals\cite{AmbulatorySys2003, EmotionalState2013}. 
Hence, multiresolution analysis techniques are essentially suitable to study the qualitative components of these 
...
This requires the time-domain signal under investigation to be convolved with a time-domain function known as “mother wavelet”. 
...
\subsubsection{Wavelet-Based Feature Extraction and SVM}
...
responses\cite{mypaper2018}. Therefore, it essentially enhances the classification performance due to 
...
a new feature space based on which an emotion classification task can be done. 
...
coefficients are calculated at different scales. Then, the amplitude of these coefficients is 
calculated to provide the corresponding spectrogram. This spectrogram is then used as the feature space.\\
...
%	\begin{center}
		\begin{tabular}{c}
			\epsfig{figure=./fig/cwt_eda.eps, scale = .6}\label{cwt_eda} \\
		\end{tabular}
...
%	\begin{center}
		\begin{tabular}{c}
			\epsfig{figure=./fig/wavelet_feature.eps, scale = .8}\label{feature} \\
		\end{tabular}
...
\begin{figure}[tbp]
	\begin{center}
		\begin{tabular}{c}
			\epsfig{figure=./fig/svm.eps, scale = .6}\label{svm} \\
		\end{tabular}
